...
class FluidCarousel extends StatefulWidget {
  ...
  final VoidCallback? onLastCardSwipe;
  final VoidCallback? onSkip;

  const FluidCarousel({super.key, required this.children, this.onLastCardSwipe, this.onSkip});
...
class FluidCarouselState extends State<FluidCarousel> with SingleTickerProviderStateMixin {
  ...
  if (_isSwipeComplete(dx, size.width)) {
    // Only trigger if swiping forward (right swipe)
    if (_index == widget.children.length - 1 && _dragDirection == -1) {
      if (widget.onLastCardSwipe != null) {
        widget.onLastCardSwipe!();
      }
    }
    return;
  }
